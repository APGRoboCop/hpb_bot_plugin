...
plugin_info_t Plugin_info = {
	...
	"HPB_Bot", // plugin name
	"4.0.5-beta", // plugin version
	"21/11/2015", // date of creation
	"botman && Pierre-Marie Baty", // plugin author
	"http://hpb_bot.bots-united.com/", // plugin URL
		  ...
		  // 1.2.5 Bots not detecting allies Fix #9
			if(	(strcmp(pkvd->szKeyName, "classname") == 0) &&
				(	(strcmp(pkvd->szValue, "info_tfdetect") == 0) ||
					(strcmp(pkvd->szValue, "info_wwdetect") == 0)
				)
			)
			{
...
void HPB_Bot_ServerCommand (void)
{
	...
	{
		// If no num then just print current count
		if( CMD_ARGC() == 3 )
		{
