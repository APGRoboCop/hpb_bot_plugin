...
int WaypointFindNearestGoal(edict_t *pEntity, int src, int team, int flags)
{
   ...
   for (int index = 0; index < num_waypoints; index++)
   {
      ...
      const int distance = WaypointDistanceFromTo(src, index, team);
...
int WaypointFindNearestGoal(const Vector& v_src, edict_t *pEntity, float range, int team, int flags)
{
   ...
   for (int index = 0; index < num_waypoints; index++)
   {
      ...
      const int distance = (waypoints[index].origin - v_src).Length();
...
{
	const int mask = W_FL_HEALTH | W_FL_ARMOR | W_FL_AMMO | W_FL_WEAPON;
      ...
      for (unsigned int x = 0; x < route_num_waypoints; x++)
      {
         for (y=0; y < route_num_waypoints; y++)
         {
            for (z=0; z < route_num_waypoints; z++)
            {
               ...
               int distance = shortest_path[y * route_num_waypoints + x] +
   ...
   for (int matrix = 0; matrix < 4; matrix++)
   {
      if (build_matrix[matrix])
      {
         ...
         if (shortest_path[matrix] == nullptr)
         {
            ...
            unsigned short* pShortestPath = shortest_path[matrix];
            unsigned short* pFromTo = from_to[matrix];
...
unsigned short WaypointRouteFromTo(int src, int dest, int team)
{
   ...
   unsigned short* pFromTo = from_to[team];
...
int WaypointDistanceFromTo(int src, int dest, int team)
{
   ...
   unsigned short* pShortestPath = shortest_path[team];
