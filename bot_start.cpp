...
// TODO: Handle starting
	if ((mod_id == TFC_DLL) || (mod_id == WIZARDWARS_DLL) || (mod_id == SWARM_DLL))
	{
		...
		if (pBot->start_action == MSG_TFC_TEAM_SELECT)
		{
			...
			// 1.2.6 Don't pick a random team, let mod decide #18
//			if (pBot->bot_team == -1)
//				pBot->bot_team = RANDOM_LONG(1, max_teams);
		...
		if (pBot->start_action == MSG_TFC_CLASS_SELECT)
		{
			...
			{
				if((pBot->bot_class <= 0) || (pBot->bot_class > SWARM_CLASSID_RANDOM))
					pBot->bot_class = SWARM_CLASSID_RANDOM;
			}
...
//TODO: Handle class limits
			...
			if((mod_id == WIZARDWARS_DLL) || (mod_id == SWARM_DLL))
			{
